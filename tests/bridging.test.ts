...
import { sendEoa, sendSmartWallet } from "./utils";
dotenv.config({ quiet: true });

import { parseUnits } from "viem";

export async function mintErUsdCrossChainFromBerachain() {
  // Chain IDs
...
export async function mintrUsdAndDepositToEulerCrossChain() {
  const BERACHAIN_ID = 80094;
  const ETHEREUM_ID = 1;
...
export async function mintrUsdAndDepositToDolomiteCrossChain() {
  const BERACHAIN_ID = 80094;
  const ETHEREUM_ID = 1;
  ...
  const DOLOMITE_DRUSD_BERACHAIN = "0x3000c6bf0aaeb813e252b584c4d9a82f99e7a71d"; // Euler vault for e-rUSD on Berachain (need actual address)
